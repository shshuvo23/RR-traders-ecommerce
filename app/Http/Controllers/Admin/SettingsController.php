...
class SettingsController extends Controller
{
    ...
    public function generalStore(Request $request)
    {
        ...
        try {
            ...
            $setting->email_enable      = $request->email_enable;
            $setting->stripe_enable     = $request->stripe_enable;
            ...
            if ($request->favicon) {
                ...
                $base_name = Str::slug($base_name); // Convert to a slug-friendly format
                $image_name = $base_name . "-" . uniqid() . "." . $favicon->getClientOriginalExtension();
                $file_path = public_path('/uploads/seo');

                if (!File::exists($file_path)) {
                    File::makeDirectory($file_path, 0755, true, true);
                }

                $favicon->move($file_path, $image_name);
                $setting->favicon = '/uploads/seo/' . $image_name;
